...
***IMPORTANTE***
Preferencialmente, cada corrida de sequenciamento deve ser pré-processada separadamente e os objetos phyloseq agrupados posteriormente para a análise.
...
### 0.3.13  Remove non-target-length sequences - aqui é feita a remoção das sequências fora do alvo de tamanho. A região v3v4 possui uma distribuição bimodal de tamanho. Por isso, é possível que sejam observados dois 'picos' de tamanhos de sequência, nas faixas de ~385 e ~407. Uma faixa segura para manter seria algo entre 380 e 415, mas é interessante inspecionar ASVs abundantes fora desta faixa para verificar sua eventual importância. 
```{r rem_non_targ, eval=TRUE, echo=TRUE}
load('nome_do_arquivo_do_ambiente_preprocessing7.RData')
table(nchar(getSequences(seqtab2)))
seqtab <- seqtab2[,nchar(colnames(seqtab2)) %in% seq(380,415)] #Se sobrarem sequências muito fora do intervalo esperado, possivelmente sejam "ruído ou sujeira". Ajustar o intervalo de tamanho de sequências para remover. 
...
export(track, "Reads through the pipeline.xlsx", rowNames = TRUE) #Exporta em excel para a pasta do projeto. 
```
...
taxtab <- assignTaxonomy(seqtab, "/home/metagenomica/Bancos_de_dados/SILVA/silva_nr99_v138.1_train_set.fa.gz")
taxtab <- addSpecies(taxtab, "/home/metagenomica/Bancos_de_dados/SILVA/silva_species_assignment_v138.1.fa.gz")
...
### 0.3.18 Merge phyloseq objects
Esta parte só deve ser utilizada caso existam múltiplos objetos ps provenientes de diferentes corridas de NGS/pre-processamento e que precisam ser agrupados para análises posteriores. Se não houver essa necessidade, esta parte pode ser removida.
```{r merge_ps, eval=TRUE, echo=TRUE}
library(phyloseq)
#Não utilizar os ps.dna, pois isso ocasionará conflito de nome dentre as ASVs.
#Copiar, colar e renomear quantas linhas forem necessárias, conforme o número de objetos ps que forem ser agrupados.
ps1 <- readRDS("/home/metagenomica/Downloads/ps1.rds") #Para evitar erros, colocar todo o caminho para o arquivo.
ps2 <- readRDS("/home/metagenomica/Downloads/ps2.rds")

ps.all <- merge_phyloseq(ps1, ps2)

#Salvando o objeto ps agrupado
saveRDS(ps.all, file = "ps.all.rds")
```
