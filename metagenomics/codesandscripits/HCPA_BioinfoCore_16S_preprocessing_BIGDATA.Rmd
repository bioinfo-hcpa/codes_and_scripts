---
title: "titulo do projeto (Script para pre-processamento de Big Data (ou dados provenientes de diferentes corridas de NGS)"
...
plotQualityProfile(fnFs1[1:6]) + ggtitle("Fwd1")
full.fwd.quality.plot1 <- plotQualityProfile(fnFs1[1:6]) + ggtitle("Fwd") #Entre chaves: coloque o intervalo total de amostras
#Salvando
...
plotQualityProfile(fnRs1[1:6]) + ggtitle("Rev1")
full.rev.quality.plot1 <- plotQualityProfile(fnRs1[1:6]) + ggtitle("Rev") #Entre chaves: coloque o intervalo total de amostras
#Salvando
...
plotQualityProfile(fnFs2[1:6]) + ggtitle("Fwd2")
full.fwd.quality.plot2 <- plotQualityProfile(fnFs2[1:6]) + ggtitle("Fwd") #Entre chaves: coloque o intervalo total de amostras
#Salvando
...
plotQualityProfile(fnRs2[1:6]) + ggtitle("Rev2")
full.rev.quality.plot2 <- plotQualityProfile(fnRs2[1:6]) + ggtitle("Rev") #Entre chaves: coloque o intervalo total de amostras
#Salvando
...
plotQualityProfile(fnFs3[1:6]) + ggtitle("Fwd3")
full.fwd.quality.plot3 <- plotQualityProfile(fnFs3[1:6]) + ggtitle("Fwd") #Entre chaves: coloque o intervalo total de amostras
#Salvando
...
plotQualityProfile(fnRs3[1:6]) + ggtitle("Rev3")
full.rev.quality.plot3 <- plotQualityProfile(fnRs3[1:6]) + ggtitle("Rev") #Entre chaves: coloque o intervalo total de amostras
#Salvando
...
### 0.3.8 Track reads through the pipeline - este chunk gera a tabela de rastreabilidade das sequências ao longo do processo.
...
track.full <- cbind(track, rowSums(seqtab.all), rowSums(seqtab.nochim))

# Adiciona a coluna sampleID ao data frame track
track$sampleID <- row.names(seqtab.nochim)
...
taxtab <- addSpecies(taxtab, "/home/local.hcpa.ufrgs.br/olovison/BancosdeDados/silva_species_assignment_v138.1.fa.gz", multithread = TRUE)
save(file = "Guerra_etal2024_preprocessing5.RData",list=c('out1', 'out2', 'out3', 'sample.names1F', 'sample.names1R', 'sample.names2F', 'sample.names2R', 'sample.names3F', 'sample.names3R', 'errF1', 'errF2', 'errF3', 'errR1', 'errR2', 'errR3', 'mergers1', 'ddF1', 'ddR1', 'mergers2', 'ddF2', 'ddR2', 'mergers3', 'ddF3', 'ddR3', 'seqtab.all', 'seqtab.nochim', 'taxtab'))
#rm(list = ls())
```
