...
x<-read.maimages(SDRF[,"File_Names"],source = "agilent", green.only = TRUE, other.columns="gIsWellAboveBG")
...
#Biblioteca anotação disponível no GEO com as entradas do Ensembl atualizadas
Annotation<-select(mgug4122a.db,keys = keys(mgug4122a.db),columns = "ENSEMBL")

#Matches de Gene.name
table$EnsemblId <- Annotation$ENSEMBL[match(table$ProbeName, Annotation$PROBEID)]
